...
char sortIDtable[] = { -1, 0,1,2,3,4,5,6,7,		17,20,
...
void SaveSortSettings(HWND hDlg)
{
	...
	SetCustTable(T("MC_sort"),itemname,itemparam,TSTRSIZE(itemparam));
}
...
void GetSortSettings(HWND hDlg,XC_SORT *xs)
{
	...
	xs->option =
		(IsDlgButtonChecked(hDlg,IDX_SORT_CASE) ?	0 : NORM_IGNORECASE) |
		(IsDlgButtonChecked(hDlg,IDX_SORT_KANA) ?	0 : NORM_IGNOREKANATYPE) |
		(IsDlgButtonChecked(hDlg,IDX_SORT_SPACE) ?	0 : NORM_IGNORENONSPACE) |
		(IsDlgButtonChecked(hDlg,IDX_SORT_SYM) ?	0 : NORM_IGNORESYMBOLS) |
		(IsDlgButtonChecked(hDlg,IDX_SORT_WIDE) ?	0 : NORM_IGNOREWIDTH) |
		(IsDlgButtonChecked(hDlg,IDX_SORT_NUM) ?	XNORM_NUMBER : 0) |
		(IsDlgButtonChecked(hDlg,IDX_SORT_LE) ?		XNORM_LASTENTRY : 0) |

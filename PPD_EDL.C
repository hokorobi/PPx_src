...
void ListWidthMain(HWND hListWnd, int dest)
{
	...
	page = SendMessage(hListWnd, LB_GETITEMHEIGHT, 0, 0) * 10;
	if ( dest > 1 ){
		width = dest;
	}else{
...
void PPeReplaceStrCommand(PPxEDSTRUCT *PES, FINDREPLACE *freplace)
{
	...
	if ( freplace->Flags & FR_FINDNEXT ){
		...
		for ( ;; ){
			...
			SendMessage(PES->hWnd, EM_REPLACESEL, 0, (LPARAM)freplace->lpstrReplaceWith); // replace後、カーソルは置換文字列の末尾に移動する
		}
...
void EditBoxExpand(PPxEDSTRUCT *PES, WindowExpandInfoStruct *eis)
{
	...
	resetflag(PES->flags, PPXEDIT_LINE_MULTI);
	...
	while( '\0' != (code = GetOptionParameter(&param, buf, &more)) ){
		if ( code == '-' ){
			...
			if ( !tstrcmp( buf + 1, T("SCROLL")) ){
				ListWidth(PES, GetNumber((const TCHAR **)&more));
				return;
			}
...
{
	if ( PES->flags & PPXEDIT_TEXTEDIT ) return 1;	// マルチなら本来のTAB
	...
	if ( (PES->list.hSubWnd != NULL) && (PES->list.ListWindow == LISTU_FOCUSSUB) ){
		PES->oldkey2 = 1;
		ListUpDown(PES->hWnd, PES, (PES->list.direction >= 0) ? -1 : 1, 0);
		return 0;
	}
...
int USEFASTCALL PPeDelete(PPxEDSTRUCT *PES)
{
	...
	if ( !( PES->flags & PPXEDIT_TEXTEDIT ) ){
		...
		if ( (PES->list.mode >= LIST_FILL) ||
			 ((X_flst[0] >= 3) && !(PES->flags & PPXEDIT_NOINCLIST) &&
				  (PES->list.hWnd == NULL)) ){
...
case K_s | K_Pup:		// \[PgUp]
case K_Pup:				// [PgUp]
	return ListPageUpDown(PES, EDITDIST_BACK);
//-----------------------------------------------
case K_s | K_Pdw:		// \[PgDw]
case K_Pdw:				// [PgDw]
	return ListPageUpDown(PES, EDITDIST_NEXT);
//-----------------------------------------------
case K_e | K_lf:			// ~[←]
	ListHScroll(PES, -1);
	break;
...
{
	if ( !(PES->flags & PPXEDIT_TEXTEDIT) ){ // 一行編集特有
		// X_flst[0]=3, 4の自動補完機能付きの一覧表示
	...
	if ( PES->flags & (PPXEDIT_LINECSR | PPXEDIT_LINE_MULTI) ){
...
LRESULT CALLBACK EDsHell(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (iMsg){
		...
		case WM_SYSCHAR:
			if ( !(PES->flags & PPXEDIT_USEALT) ) break;	// Alt 系の使用を禁止
		//	WM_CHAR へ
		...
		case WM_GETDLGCODE:
			...
			if ( !(PES->flags & PPXEDIT_TABCOMP) ) break;
...
void InitExEdit(PPxEDSTRUCT *PES)
{
	...
	if ( PES->style & ES_MULTILINE ){
		if ( PES->flags & PPXEDIT_LINE_MULTI ){
			...
			if ( (WinType >= WINTYPE_VISTA) && (PES->flags & PPXEDIT_NOWORDBREAK) ){
				SendMessage(PES->hWnd, EM_SETWORDBREAKPROC, 0, (LPARAM)LineEditWordBreakProcW);
			}
		}
	}
...
*/
		if ( !( PES->flags & PPXEDIT_TEXTEDIT ) && (X_flst[0] == 2) ){

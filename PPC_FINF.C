...
static const TCHAR *AttrsLabels[ATTR_FLAGS_COUNT] = {
	...
	T("Virtual"), T("No scrub data"), T("Recall on open"), T("Pinned"),
	T("Unpinned"), T("B21"), T("Recall on data access"), T("B23"),
	T("B24"), T("B25"), T("B26"), T("B27"),
	T("B28"), T("Strictly sequential"), T("B30"), T("B31")
};
		...
		if ( hFile != INVALID_HANDLE_VALUE ){
			...
			if ( DGetFileInformationByHandleEx != NULL ){
				...
				if ( IsTrue(DGetFileInformationByHandleEx(hFile,
					...
					tstrcpy(dst, T("\r\nChange Time   :"));
					dst += tstrlen(dst);
					dst += CnvDateTime(dst, NULL, NULL, (FILETIME *)&((xFILE_BASIC_INFO *)buf)->ChangeTime);
				}
				if ( IsTrue(DGetFileInformationByHandleEx(hFile,
...
// ※ LowPart/HighPart が使えないため、直接指定している
					FormatNumber(dst, XFN_SEPARATOR, 26,
						*(DWORD *)(&((xFILE_STANDARD_INFO *)buf)->AllocationSize),
						*((DWORD *)(&((xFILE_STANDARD_INFO *)buf)->AllocationSize) + 1));
					dst += tstrlen(dst);
				}
			}
			text->top = (char *)dst - text->bottom;

...
DWORD_PTR USEFASTCALL PPcExecExMenu(PPC_APPINFO *cinfo,EXECEXMENUINFO *execmenu)
{
	...
	}else if ( (index >= CRID_DIROPT) && (index <= CRID_DIROPT_THIS_BRANCH) ){
...
void MakeListFileCommand(PPC_APPINFO *cinfo,const TCHAR *param)
{
	...
	if ( NextParameter(&param) ){
		mode = GetStringCommand(&param,T("NORMAL\0") T("MARKED\0") T("MARKTAG\0") T("NAME\0") );
	}
	WriteListFileForUser(cinfo,path,mode);
}
...
void PPvOptionCommand(PPC_APPINFO *cinfo,const TCHAR *param)
{
	...
	if ( (tstrcmp(buf,T("sync")) == 0) || (tstrcmp(buf,T("setsync")) == 0) ){
		...
		if ( tstrcmp(buf,T("setsync")) == 0 ){
			...
			SetCustTable(T("_others"),T("SyncViewID"),buf,TSTRSIZE(buf));
		}
